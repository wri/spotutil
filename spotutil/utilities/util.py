...
def launchtime(launch):
    ...
    launch_time = launch.astimezone(
        to_zone
    )  # datetime.datetime(2018, 7, 19, 8, 44, tzinfo=<DstTzInfo 'US/Eastern' EDT-1 day, 20:00:00 DST>)
    ...
    tdelta = datetime.strptime(now_f, time_fmt) - datetime.strptime(
        launch_time_f, time_fmt
    )
